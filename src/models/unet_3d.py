...
from .unet_3d_lora import UNet3DConditionLoadersMixin
...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet3DConditionLoadersMixin):
    ...
    def forward(
        ...
        Args:
            ...
            encoder_hidden_states (`torch.FloatTensor`): (batch, frame, feature_dim) encoder hidden states
        ...
        # pre-process
        sample = self.conv_in(sample)   # torch.Size([2, 320, 24, 96, 64])
        if pose_cond_fea is not None:
            sample = sample + pose_cond_fea     # torch.Size([2, 320, 24, 96, 64])
